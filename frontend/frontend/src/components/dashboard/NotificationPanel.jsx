  ...
  return (
    <div className="relative" ref={ref}>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="relative flex h-9 w-9 items-center justify-center rounded-full text-navy-700/60 hover:bg-navy-50"
      ...
      {open && (
        <div className="absolute right-0 z-20 mt-2 w-80 overflow-hidden rounded-2xl bg-white shadow-xl ring-1 ring-navy-900/8">
          <div className="flex items-center justify-between border-b border-navy-900/8 px-4 py-3">
            ...
            {unreadCount > 0 && (
              <button type="button" onClick={handleMarkAllRead} className="text-xs font-semibold text-brand-500 hover:text-navy-900">
                Mark all read
              </button>
            )}
          </div>
          ...
          <div className="max-h-80 overflow-y-auto">
            {loading ? (
              <p className="px-4 py-8 text-center text-sm text-navy-700/45">Loading…</p>
            ) : notifications.length === 0 ? (
              <p className="px-4 py-8 text-center text-sm text-navy-700/45">You're all caught up.</p>
            ) : (
              notifications.map((n) => (
                <div key={n.id} className="flex items-start gap-3 border-b border-navy-900/6 px-4 py-3 last:border-0">
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-navy-900">{n.title}</p>
                    <p className="mt-0.5 line-clamp-2 text-xs text-navy-700/60">{n.message}</p>
                    <p className="mt-1 text-[10px] text-navy-700/40">{timeAgo(n.created_at)}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => handleMarkRead(n.id)}
                    className="mt-0.5 shrink-0 rounded-full p-1 text-navy-700/40 hover:bg-navy-50 hover:text-brand-500"
