    ...
    <div>
      <div
        className={`flex items-center justify-between gap-3 rounded-xl border border-dashed px-4 py-3 text-sm ${
          displayError ? 'border-red-300 bg-red-50/40' : 'border-navy-900/15 bg-navy-50/40'
        }`}
      >
        <div className="min-w-0">
          <button
            ...
            className="font-semibold text-brand-500 hover:text-navy-900"
          >
            {label || 'Choose file'}
          </button>
          <p className="mt-0.5 truncate text-xs text-navy-700/50">
            {value ? (typeof value === 'string' ? value.split('/').pop() : value.name) : hint || 'No file selected'}
          </p>
        </div>
        {value && (
          <button type="button" onClick={clear} aria-label="Remove file" className="shrink-0 text-navy-700/40 hover:text-red-600">
