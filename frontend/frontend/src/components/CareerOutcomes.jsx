        ...
        <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-5">
          {steps.map((step, i) => (
            <div key={step.label} className="animate-fade-up relative text-center" style={{ animationDelay: `${i * 100}ms` }}>
              {i < steps.length - 1 && (
                <div className="absolute left-1/2 top-7 hidden h-px w-full bg-white/10 lg:block" />
              )}
              <div className="relative mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-white/10 text-white ring-1 ring-white/15 transition-transform duration-300 hover:scale-110">
                <step.icon className="h-6 w-6" />
              </div>
              <h3 className="mt-4 text-sm font-bold text-white">{step.label}</h3>
              <p className="mt-1.5 text-xs leading-relaxed text-navy-100/60">{step.description}</p>
            </div>
          ))}
        </div>
      </div>
