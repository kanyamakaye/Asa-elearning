...
const pointKeys = ['unlimitedCourses', 'liveClasses', 'grading', 'getPaid']
        ...
        <div className="grid items-center gap-12 rounded-3xl bg-gradient-to-br from-navy-800 to-navy-900 px-8 py-14 lg:grid-cols-2 lg:px-16">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-brand-300">
              {t('public.instructorCta.eyebrow')}
            </p>
            <h2 className="mt-3 text-3xl font-extrabold tracking-tight text-white sm:text-4xl">
              {t('public.instructorCta.heading')}
            </h2>
            <p className="mt-4 text-navy-100/70">
              {t('public.instructorCta.description')}
            </p>
            <Link
              to="/signup"
          ...
          <ul className="grid gap-4">
            {pointKeys.map((key) => (
              <li key={key} className="flex items-start gap-3 rounded-xl bg-white/5 p-4 ring-1 ring-white/10">
                <div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-brand-500 text-white">
                  <IconCheck className="h-3.5 w-3.5" />
                </div>
                <span className="text-sm font-medium text-navy-100/85">{t(`public.instructorCta.points.${key}`)}</span>
              </li>
            ))}
          </ul>
        </div>
